...
show_image(idx=3, train=False) ##### Displays the image with its corresponding captions 


##################### Now import a pre-trained VGG16 model with weights of imagenet imported. 
### This would save iteration time by almost 25 mins per epoch if VGG net has to be trained by our dataset itself.

image_model = VGG16(include_top=True, weights='imagenet')

image_model.summary()

## Now extract the weights at the 2nd fully connected dense layer to transfer it to RNN network for caption generation
transfer_layer = image_model.get_layer('fc2')

image_model_transfer = Model(inputs=image_model.input,
                             outputs=transfer_layer.output)
print(image_model.input)
## Output - <tf.Tensor 'input_1:0' shape=(?, 224, 224, 3) dtype=float32>
print(transfer_layer.output)
## Output - <tf.Tensor 'fc2/Relu:0' shape=(?, 4096) dtype=float32>

### Therefore the model expects the input shape as seen below:
img_size = K.int_shape(image_model.input)[1:3]      # K is keras here
print(img_size) 
# Output - (224, 224)
...
def process_images(data_dir, filenames, batch_size=5):
    """
...
captions_val_flat = flatten(captions_val_marked)
num_words = 10000
...
tokenizer = TokenizerWrap(texts=captions_val_flat,
                          num_words=num_words)
...
def get_random_caption_tokens(idx):
    ...
    for i in idx:
        ...
        j = np.random.choice(len(tokens_val[i]))

        # Get the j'th token-sequence for image i.
        tokens = tokens_val[i][j]
...
def batch_generator(batch_size):
    ...
    while True:
        ...
        idx = np.random.randint(num_images_val,
                                size=batch_size)
...
steps_per_epoch = int(total_num_captions_val / batch_size)
...
### Now create a recurrent neural network for caption generation
...
###################### Train the model ##############
mod = decoder_model.fit_generator(generator=generator,
                            steps_per_epoch=steps_per_epoch,
                            epochs=2,
                            callbacks=callbacks)

###### Generate Captions ######################33
...
generate_caption_coco(idx=100, train=False)     #Test for idx=5
